...
| `reverse` | `false` | Reverse the inline commit selector's order. `false` shows the head end on top (tuicr's default); `true` shows the base end on top (parent → child), like `git log --reverse`. Equivalent to the `--reverse` CLI flag. Affects display and navigation only — not the diff or saved sessions. |
